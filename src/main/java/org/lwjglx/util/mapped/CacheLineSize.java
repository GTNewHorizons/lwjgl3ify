...
import static org.lwjglx.util.mapped.MappedHelper.ivget;
import static org.lwjglx.util.mapped.MappedHelper.ivput;
...
final class CacheLineSize {
    ...
    static int getCacheLineSize() {
        ...
        final int MAX_SIZE = LWJGLUtil.getPrivilegedInteger("org.lwjglx.util.mapped.CacheLineMaxSize", 1024) / 4; // in
                                                                                                                  // #
                                                                                                                  // of
                                                                                                                  // integers
        // Detection will stop when the execution time increases by more than CacheLineTimeThreshold %.
        final double TIME_THRESHOLD = 1.0
            + LWJGLUtil.getPrivilegedInteger("org.lwjglx.util.mapped.CacheLineTimeThreshold", 50) / 100.0;
        ...
        try {
            ...
            final int WARMUP = 10;
            for (int i = 0; i < WARMUP; i++) doTest(THREADS, LOCAL_REPEATS, 0, memory, completionService);
            ...
            if (LWJGLUtil.DEBUG) {
                if (found) LWJGLUtil.log("Cache line size detected: " + cacheLineSize + " bytes");
                else LWJGLUtil.log("Failed to detect cache line size, assuming " + cacheLineSize + " bytes");
            }
    ...
    private static long doTest(final int threads, final int repeats, final int padding, final IntBuffer memory,
        ...
        for (int i = 0; i < threads; i++) submitTest(completionService, i, repeats, memory, padding);
    ...
    private static long waitForResults(final int count, final ExecutorCompletionService<Long> completionService) {
        try {
            long totalTime = 0;
            for (int i = 0; i < count; i++) totalTime += completionService.take()
                .get();
            return totalTime;
        } catch (Exception e) {
