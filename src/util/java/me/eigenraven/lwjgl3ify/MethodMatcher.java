...
public class MethodMatcher {
    ...
    public MethodMatcher(
            JarApiSet oldApi,
            JarApiSet newApi,
            Map.Entry<String, JarApiSet.ClassApi> oldClassNode,
            Map.Entry<String, JarApiSet.ClassApi> newClassNode,
            Map.Entry<String, MethodNode> oldMethodNode) {
        this.oldApi = oldApi;
        this.newApi = newApi;
    ...
    public MethodRedirector findMatch() {
        ...
        if (nativeCallName != null) {
            ...
            final Map.Entry<String, MethodNode> fuzzyMatch =
                    newClassNode.getValue().methods.ceilingEntry(newKey);
                ...
                if (newArgTypes.length == argTypes.length) {
                    ...
                    for (int i = 0; i < newArgTypes.length; i++) {
                        ...
                        if (!oldArgType.equals(newArgType)) {
                            if (newArgType.startsWith("java.nio.")
                                ...
                                changedArgs[i] =
                                        "me.eigenraven.lwjgl3ify.BufferCasts.bufferToCharSeq(" + argNames[i] + ")";
                            } else if (newArgType.equals("int")
                                    && oldArgType.equals("boolean")
                                ...
                                switch (bufferArg) {
                                    ...
                                    case "java.nio.LongBuffer":
                                        newArg =
                                                "(unsigned ? org.lwjgl.opengl.GL11.GL_UNSIGNED_LONG : org.lwjgl.opengl.GL11.GL_LONG)";
                                        break;
                                    case "java.nio.FloatBuffer":
                                        newArg =
                                                "(unsigned ? org.lwjgl.opengl.GL11.GL_UNSIGNED_FLOAT : org.lwjgl.opengl.GL11.GL_FLOAT)";
                                        break;
                                    case "java.nio.DoubleBuffer":
                                        newArg =
                                                "(unsigned ? org.lwjgl.opengl.GL11.GL_UNSIGNED_DOUBLE : org.lwjgl.opengl.GL11.GL_DOUBLE)";
                                        changedArgs[bufferArgIdx] = "me.eigenraven.lwjgl3ify.BufferCasts.toByteBuffer("
                                                + changedArgs[bufferArgIdx] + ")";
                                        break;
                                    default:
                                ...
                                success = false;
                                System.out.println(new UnsupportedOperationException(oldArgType + " -> " + newArgType
                                                + " in " + oldClassNode.getKey() + " : " + oldMethodNode.getKey())
                                        .toString());
                            }
                        }
                    }
                    if (success) {
                        return new MethodRedirector.TransformingRedirector(
                                this, fuzzyMatch.getValue(), preCode.toString(), postCode.toString(), changedArgs);
                    }
                }
            }
        }
